...
✅ **完了**: 8. 7.が完了後、LCDを仕様にのっとった実装にする事
   ...
   - 画面中央揃え表示の実装
9. バッテリー表示 左上にバッテリー表示を追加する事。バッテリー表示は%表示とする事
10. IMUのデータを取得する事。データ取得周期はRefelenceのリポジトリを参考にする事。
11. GPSのデータを取得する事。データ取得周期はRefelenceのリポジトリを参考にする事。
12. GPSとIMUデータを録画時にOsmo360に送信すること。Protocolや使用方法はRefelenceのリポジトリのドキュメントとサンプルプログラムを参考にすること。
13. Wifi等の使っていないペリフェラルの電源を落として省電力化する事
14. LCDのデザインをUI/UXに基づき改善する事。画面が小さいことを考慮し、必要なものを大きく表示し視認性を高め、画面の占有率をなるべく上げる。また、必要性が低いものは小さく表示するなどデザインを熟考し、改善する事。
