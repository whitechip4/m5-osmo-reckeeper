...
一度録画開始が検知された後、ボタンAを押したとき以外で録画が停止判定になった場合、自動で再び録画開始信号を送る機能。

* ボタンBによってRecKeepModeの有効/無効を切り替える
* RecKeepModeがONの場合は、録画の停止が検知された場合、n秒後に再び録画開始信号を送る
  * つまり外部から録画が停止された場合のみ動作する
  * 故にRecKeepModeがONの状態でも、本デバイスからの信号で録画が停止された場合は録画の再開始は行わない
* RecKeepModeの状態は録画開始時/停止時などのイベントで不揮発領域に書き込み、起動時に復元させる

### LCD表示内容

LCDディスプレイは左に90度回転させ横向きに使う。また、表示内容や詳細は以下。
RecKeepモードのON/OFFを右上に小さく表示する。

* コンテクストごとに行を変えて描画して良い。UX的に自然に
* LCDから文字がはみ出さないように座標を調整する
* LCDの表示は一番優先度を低くし、他の処理に影響が出ないようにする
* LCDの表示内容は
  * READY状態… PUSH TO Pairingと文字を表示。
  * ペアリング試行中… Finding Device... と（ID)を表示
  * ペアリング後
    * 録画停止中ならば　■STOP を表示。二行目にPress to start を表示。
    * 録画中なら赤い ●REC を表示。また、録画時間を表示。
      * 録画時間はカメラから取得可能ならそれを利用、出来なければ内部時間を使用

### 電源OFFボタン

* PWRボタンの押下を検知した場合、
  * 単押しの場合、デバイスのResetをかける
  * 長押しの場合あとn秒間押し続けると電源が落ちる旨を表示する
