...
```
main/           - メインソースコード
  main.c         - エントリーポイント、イベントループ、ボタン処理
  ...           - TBD
components/     - M5Stack 公式ライブラリ (M5Unified, M5GFX)
reference/      - DJI 公式デモ (Submodule、参考実装)
...
* ビルドメッセージの確認でトークンを消費しすぎないように適宜grepやtailを使って行数を絞って読むこと。
...
ESP-IDF 標準の NimBLE (`esp-nimble`) を使用する。
実装は `main/ble_simple.c` にあり、DJI 公式デモの BLE 層を参考にしています。
...
現在の実装では、referenceのprotocol/ディレクトリは使用せず、
必要な機能のみを `dji_frame.c` で独自実装しています（録画制御とステータス取得）。

### 実装の状態

現在の実装はシングルスレッドのイベントループ方式を採用しており、
main.cのapp_main()内でボタン処理とBLEイベントをpollingしています。
FreeRTOSのタスクはシステムタスクのみを使用し、アプリケーション固有のタスクは作成しません。
