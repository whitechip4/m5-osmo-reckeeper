...
```
main/           - メインソースコード (C/C++)
  main.c         - エントリーポイントとイベントループ
  m5_wrapper.cpp - M5Unified C++ APIのCラッパー
  dji_protocol.c - DJIプロトコル実装
  ble_simple.c   - BLE通信処理
  gps_module.c   - GPSデータ取得
  storage.c      - NVSストレージ処理
  config.h       - 設定値
  display/       - ディスプレイヘルパー関数
  handlers/      - ボタン処理、ステータス更新、システム初期化
  protocol/      - プロトコルパーサー、CRC計算
  ui/            - UIレンダリング、状態管理、レイアウト
  CMakeLists.txt - コンポーネント依存設定
components/     - 使用ライブラリ (git submodule)
  M5Unified/     - ハードウェア抽象化レイヤー
  M5GFX/        - LCD/グラフィック描画ライブラリ
reference/      - 参考実装 (git submodule)
